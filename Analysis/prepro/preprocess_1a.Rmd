...
df.image1 <- df.mismatch %>%
  filter(Image == "img/S_ambi40.png") %>%
  select(word,subj_idx) 
unique(df.image1)
df.image2 <- df.mismatch %>%
  dplyr::filter(image == "img/C_ambi40.png")
df.image3 <- df.mismatch %>%
  dplyr::filter(image == "img/H_ambi40.png")
...
df_acc
df_acc <- df_acc %>%
  select(subj_idx,matchness,selfness,ACC)%>%
  pivot_wider(names_from = c("selfness","matchness"),values_from = "ACC")
...
df_desc %>% 
  select(-n) %>% 
pivot_wider(names_from = c("selfness", "matchness"), values_from = "rt") %>% 
  write_csv("df.csv")
